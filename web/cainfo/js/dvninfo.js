...
function moveEditFocus() {
    ...
    if (obj == null)
        return;
    obj.style.left = (curEditFocus * 30 + 827) + 'px';
...
//	rating = DVB.getDVNRating();
	rating = caInfo['Rating'];
...
function showAuthInfo() {
	...
	for ( var i = 0; i < 2; i++) {
        if (i < infoList.length) {
            ...
            document.getElementById("operid" + i).innerHTML = infoList[i].num;
...
function showCAInfo(  ) {
    ...
    var info = DVB.getCAInfo();

	caInfo = eval("(" + info  + ")");
	

	
    setCtrlText('cardID', caInfo['cardnum']);
    setCtrlText('version', "V"+caInfo['version']);
    setCtrlText('STBatrr', "HD");
    setCtrlText('STBName', caInfo['STBname']);
    setCtrlText('cakTime', caInfo['date']);
    setCtrlText('cakVersion', caInfo['cakVersion']);
	setCtrlText('pocket', caInfo['pocket'].toFixed(2) + "元");
   	setCtrlText('paired',  caInfo['paired'] ? "已配对":"未配对");
...
function dealPinKey(keycode) {
    ...
    case ENTER:
        ...
        if( oldPin.length != 4 ){
            readyToChange = false;
            askDlg.popDlg('修改PIN码', '输入的旧PIN码的长度不是4位，请重新输入！', askPinHandle);
        }else if( newPin != repeat ){
            readyToChange = false;
            askDlg.popDlg('修改PIN码', '再次输入的新PIN码不一致，请重新输入！', askPinHandle);
        }else if( newPin.length != 4 || repeat.length != 4 ){
            readyToChange = false;
            askDlg.popDlg('修改PIN码', '新PIN码的长度不是4位！', askPinHandle);
        }else{
            readyToChange = true;
...
function dealRatingKey( keyCode )
{
    ...
    case ENTER:
        ...
        if( pin.length != 4 ){
            askDlg.popDlg('修改观看级别', '输入的PIN码的长度不是4位，请重新输入！', askRatingHandle);
             isAsking = true;
        }else{
            changeRating( pin );
        }
...
function dealWorkTimeKey(keyCode)
{
    ...
    case ENTER:
        ...
        if( startHour > 23 || endHour > 23 || endMinute > 59 || startMinute > 59){
           ...
           // var ret = DVB.modifyWorkTime(pin, startHour, startMinute, endHour, endMinute );
            if( msgBox == null ){
...
function moveRight() {
    ...
    if (!isLeft) {//右边
        ...
        if (bannerFocus == 2 && infoList.length > 0) {
            ...
            inputText0 = new Array(4);
            inputText1 = new Array(4);
            inputText2 = new Array(4);
            curInputFocus = 0;
            curEditFocus = 0;
            moveInputFocus();
            moveEditFocus();
...
function dealKey(keyCode) {
    ...
    case ENTER:
        if (!isLeft && bannerFocus == 2 && infoList != null) {
			...
			var id = infoList[listFocus].name;
...
function changeRating( pin)
{
    ...
    if( ret == 0){
        ...
        msgBox.popMessage('修改观看级别', "机卡通讯错误", msgBoxHandle);
        showInfo = true;
    }else if( ret == 2){
        msgBox.popMessage('修改观看级别', "智能卡不支持此功能", msgBoxHandle);
        showInfo = true;
    } else if( ret == 3){
        msgBox.popMessage('修改观看级别', "PIN码输入不正确", msgBoxHandle);
        showInfo = true;
    }
...
function askPinHandle( sel )
{
    ...
    if (sel == true && readyToChange ){
        ...
        if( ret == 0){
            ...
            msgBox.popMessage("CA温馨提示","智能卡通讯错误", msgBoxHandle);
            showInfo = true;
        }else if (ret == 2)
		{
			msgBox.popMessage("CA温馨提示","智能卡不支持此功能", msgBoxHandle);
            showInfo = true;
		}
		else if (ret == 3){
            msgBox.popMessage("CA温馨提示","PIN码格式错误", msgBoxHandle);
            showInfo = true;
        }
    }
    readyToChange = false;
